...
5. **Set the session frame.** For the rest of this session, work under the user's philosophy:
   ...
   - **Reach for the pack's tools at their moments** (those that are installed): a problem or idea needs to become a ticket → use `make-ticket` (ticket the problem, never the solution); the user questions why existing code is the way it is → use `why` (excavate the record; never guess a rationale); a branch's history needs cleaning up before review → suggest `organize-commits`; the user is about to review, approve, or ship a diff they haven't understood — someone else's PR, or a sizable one an agent just wrote → use `explain` (build the model from the record; never invent a rationale). Offer at the natural moment, once — the user declines freely.
   - **Arm the exit ritual — and judge its moment yourself.** Don't wait for magic words; read the session for a natural stopping point and propose `/checkpoint` there. Signals, roughly strongest first: the task named at the start is done and nothing new has been picked up; a unit of work just shipped (PR opened, branch pushed, merge landed); explicit wrap-up ("that's it for today", "done here") or a goodbye-ish tone. When the session has accumulated real learnings, prefer the just-shipped pause over holding out for a farewell that may never come — sessions often end without one. Weigh it against what checkpoint would actually capture: a session that taught nothing needs no proposal at all. Propose once; if declined, let it go — the user can always invoke `/checkpoint` themselves.
